#!/usr/bin/env python3
"""Create BMC Health Services Research formatted English manuscript.

Conforms to BMC Health Services Research original research author guidelines:
- Title should indicate study design
- Structured abstract (Background/Methods/Results/Conclusions; optional Trial registration)
  <=350 words
- Keywords: 3-10
- Body concise; no strict word limit but approximately <=4000 words
- References numbered in order of appearance (Vancouver style; in-text square brackets)
- Unnumbered IMRaD sections and subsections
- Double-spaced, Times New Roman 12 pt
- Declarations section in the main manuscript (ethics, data availability, competing
  interests, funding, authors' contributions, acknowledgements)
- Separate title page with author information and manuscript metadata
- STROBE checklist for cross-sectional studies as supplementary material
- Cover letter required, explaining fit for BMC Health Services Research
...
def build_flat(R):
    ...
    F['zenodo_doi'] = ZENODO_DOI if ZENODO_DOI else '[Zenodo DOI to be assigned]'
    return F
...
# BMC Health Services Research uses single-anonymised peer review.
...
add_para("Reporting guideline: STROBE checklist for cross-sectional studies (uploaded as Supplementary file 1)", italic=True)
add_para("Target journal: BMC Health Services Research", italic=True)
...
add_para(
    ...
    "universal health coverage; health services research; Japan")
...
add_para(
    ...
    "studies,{30} which is provided as Supplementary file 1.")
...
add_subheading("Ethics approval and consent to participate")
add_para(
    "This study used publicly available aggregate data only. Ethics committee "
    "approval was not required under the Ethical Guidelines for Medical and "
    "Biological Research Involving Human Subjects (Japan, 2021 revision).")

add_subheading("Consent for publication")
add_para("Not applicable.")

add_subheading("Availability of data and materials")
add_para(
    "All data are publicly available. Standardised claim ratios: Cabinet Office "
    "Regional Variation Visualisation "
    "(https://www5.cao.go.jp/keizai-shimon/kaigi/special/reform/mieruka/chiikisa/index.html). "
    ...
    "and archived on Zenodo (https://doi.org/${zenodo_doi}).")

add_subheading("Competing interests")
add_para("[To be completed by the authors.] No known competing interests.")

add_subheading("Funding")
add_para("[To be completed by the authors.] This work received no specific grant.")

add_subheading("Authors\' contributions")
add_para("[To be completed by the authors.]")

add_subheading("Use of AI tools")
add_para(
    "During the preparation of this manuscript the authors used Devin (Cognition AI) "
    "as a coding assistant for data processing, statistical modelling, geographic "
    ...
    "integrity and accuracy of the published work.")

add_subheading("Acknowledgements")
add_para(
    "The authors thank the Cabinet Office Regional Variation Visualisation "
    "initiative, the Ministry of Health, Labour and Welfare, and the Ministry "
    "of Land, Infrastructure, Transport and Tourism for maintaining the public "
...
for p in doc.paragraphs:
    ...
    if txt.upper() == 'INTRODUCTION':
        ...
        in_body = True
        continue
    # BMC HSR places the Declarations section before References; it should not
    # be counted as part of the main scientific body.
    if txt.upper() == 'DECLARATIONS':
        in_body = False
        continue
    if txt.upper() == 'REFERENCES':
        in_body = False
...
# Save
# ============================================================
out = os.path.join(OUTPUT_DIR, 'regional_anaesthesia_BMC_HSR_EN.docx')
doc.save(out)
print(f"Saved: {out}")

# --------------------------------------------------
# Separate title page for BMC Health Services Research
# --------------------------------------------------
tp = Document()
...
tp_para("Reporting guideline: STROBE checklist for cross-sectional studies (Supplementary file 1)", italic=True)
tp_para("Target journal: BMC Health Services Research", italic=True)

tp.add_paragraph()
tp_para(
    "Declarations: A complete Declarations section is included in the main "
    "manuscript (Ethics approval and consent to participate; Consent for "
    "publication; Availability of data and materials; Competing interests; "
...
tp_out = os.path.join(OUTPUT_DIR, 'title_page_BMC_HSR_EN.docx')
tp.save(tp_out)
...
meta_path = os.path.join(OUTPUT_DIR, 'bmc_hsr_submission_metadata.json')
