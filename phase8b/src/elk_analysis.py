...
def main():
    ...
    u = u[u.groupby("ind").ym.transform("nunique") >= 24]
    ...
    for ind, d in u.groupby("ind"):
        ...
        rows.append(r)
    hg = pd.DataFrame(rows); hg.to_csv(OUT / "elk_history_gain.csv", index=False)
    # decay
    dec = []
    for ind, d in u.groupby("ind"):
        for h in [1, 2, 3, 5, 12]:
            dec.append({"ind": ind, "horizon": h,
                        "oos": loyo(d.u.values, d[envcols + [f"u_lag{h}" if h != 12 else "u_lag12"]].values,
                                    d.unit.values, d.year.values)})
    pd.DataFrame(dec).to_csv(OUT / "elk_memory_decay.csv", index=False)
    ...
    pd.DataFrame(me).to_csv(OUT / "elk_matched_env.csv", index=False)
