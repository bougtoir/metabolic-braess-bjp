#!/usr/bin/env python3
"""
Generate Nature submission package (double-blind):
  - manuscript_nature_doubleblind.docx (no author info)
...
# ---------------------------------------------------------------------------
# Content (imported from create_nature_manuscript.py constants)
# ---------------------------------------------------------------------------
...
ABSTRACT = (
    ...
    "Extending to fully three-dimensional scattering with 2.5 post-Newtonian "
    "gravitational-wave radiation reaction confirms that the PK mapping is "
    "robust across dimensionality and dissipation. "
...
def build_doubleblind_manuscript():
    ...
    p = doc.add_paragraph()
    run = p.add_run(
        "All simulation data and analysis code will be made publicly available "
        "upon publication at [repository URL to be provided after acceptance]."
    )
    run.font.size = Pt(11)
    ...
    p = doc.add_paragraph()
    run = p.add_run(
        "The Julia N-body simulation code, Python analysis scripts, and "
        "manuscript generation code will be deposited in a public repository "
        "upon acceptance. [URL to be provided after review.]"
    )
    run.font.size = Pt(11)
    ...
    out_path = os.path.join(OUTDIR, "manuscript_nature_doubleblind.docx")
    doc.save(out_path)
...
def build_cover_letter():
    ...
    run = date_p.add_run("27 June 2026")
    run.font.size = Pt(11)
    ...
    paras = [
        ...
        ("We submit the above manuscript for consideration as an Article in Nature. "
         "This work establishes that chaotic three-body gravitational scattering "
         "and pharmacokinetic compartment theory share an exact mathematical "
         "structure\u2014continuous-time Markov chains on three states with "
         "irreversible exit\u2014and that the analytical tools of each field "
         "can be productively imported into the other."),
    ...
    bullets = [
        ("A linear 3-compartment PK model reproduces the multi-exponential "
         ...
         "15,000 N-body simulations (3 mass configurations, KS test p < 0.001 "
         "versus single exponential)."),

        ("A nonlinear (Michaelis-Menten) PK extension captures the power-law "
         "tail from sticky chaos (36\u201342% tail RMSE improvement), providing "
         "the first minimal parametrisation of the ergodic-to-trapped transition."),

        ("Population PK (mixed-effects) modelling yields closed-form allometric "
         "scaling of mean residence time with mass ratios across 64 configurations "
         "(MRT \u221d \u03bc\u2081\u2082\u00b2\u00b7\u2070\u2078 \u00b7 "
         "\u03bc_out\u207b\u00b9\u00b7\u2076\u2078, R\u00b2 = 0.67)."),

        ("The reverse mapping provides a stability analysis framework for "
         "target-mediated drug disposition (TMDD) grounded in celestial mechanics."),
    ...
    paras2 = [
        ("We believe this work is appropriate for Nature because it reveals "
         "an unexpected structural connection between two major fields of "
         "science\u2014gravitational dynamics and pharmacology\u2014that is "
         "quantitative, bidirectional, and immediately actionable by both "
         "communities. The three-body problem has been a central challenge in "
         "physics for over 250 years; pharmacokinetic compartment modelling is "
         "the foundation of modern drug development. Showing that these are "
         "the same mathematical problem, and that tools from each field solve "
         "open questions in the other, will be of broad interest to Nature's "
         "readership."),
    ...
    run.font.size = Pt(11)
    run = excl_p.add_run(
        "Nathan W.C. Leigh (Universidad de Concepcion), "
    ...
    out_path = os.path.join(OUTDIR, "cover_letter.docx")
    doc.save(out_path)
    print(f"Saved: {out_path}")
    return out_path
...
if __name__ == "__main__":
    print("=" * 60)
    print("  Generating Nature Submission Package (Double-Blind)")
    print("=" * 60)
