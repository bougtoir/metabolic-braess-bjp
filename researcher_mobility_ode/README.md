...
This installs dependencies from `requirements.txt` (Python 3.10+, `pandas`, `numpy`, `scipy`, `requests`, `matplotlib`, `python-docx`, `python-pptx`, `Pillow`) and regenerates all `results/`, figures, and manuscript documents from the included `data/cohort/` files.
