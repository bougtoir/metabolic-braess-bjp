...
def load_references(source_name=None):
    """Parse REFERENCES list from a manuscript script without executing it."""
    source = SCRIPT_DIR / (source_name or "create_perspective_docx.py")
    content = source.read_text(encoding="utf-8")

    # Extract the REFERENCES list using regex
    match = re.search(r'^REFERENCES\s*=\s*\[(.*?)\n\]', content, re.MULTILINE | re.DOTALL)
    if not match:
        raise RuntimeError(f"Could not find REFERENCES list in {source.name}")
...
def _normalize_surname(token):
    """Return a lowercase, ASCII-only surname key for fuzzy matching.

    Removes diacritics, non-ASCII hyphens, and other punctuation so that
    e.g. 'Izuel-Idoype' and 'Izuel‐Idoype' (U+2011 non-breaking hyphen)
    map to the same comparison key.
    """
    token = token or ""
    # Normalize unicode characters to closest ASCII, then drop anything
    # that is not a basic Latin letter.
    import unicodedata
...
def extract_ms_author_surnames(authors_raw, limit=4):
    """Extract surnames from a Vancouver author string before 'et al.'.

    Returns comparison keys for the first author and up to `limit - 1`
    co-authors. Surnames are the first whitespace-delimited token of each
    author entry.
    """
    if not authors_raw:
        return []
    ...
    surnames = []
    for p in parts:
        if p.lower().startswith('et al'):
            break
        tokens = p.split()
        if not tokens:
            continue
        surname = _normalize_surname(tokens[0])
        if surname:
...
def extract_cr_author_surnames(authors_crossref):
    """Return comparison keys from Crossref author strings (Family Initials)."""
    surnames = []
    for a in authors_crossref or []:
        if not a:
            continue
        surname = _normalize_surname(a.split()[0])
        if surname:
            surnames.append(surname)
    return surnames
...
def compare_reference(ref_idx, ref_str, ref_info, crossref_result):
    ...
    # ── Author checks (first author + first few co-author surnames) ──
    first_author_ms = _normalize_surname(ref_info.get("first_author_surname", ""))
    authors_cr = crossref_result.get("authors_crossref", [])
    cr_surnames = extract_cr_author_surnames(authors_cr)
    if first_author_ms and cr_surnames:
        first_author_cr = cr_surnames[0]
        if first_author_ms != first_author_cr:
            ...
            # If first author is already wrong, do not also report co-author mismatches
            first_author_ok = False
        else:
            first_author_ok = True

        # Cross-check the first few co-author surnames beyond the first author
        if first_author_ok:
            ms_surnames = extract_ms_author_surnames(ref_info.get("authors_raw", ""), limit=4)
            if len(ms_surnames) > 1:
                mismatched = [s for s in ms_surnames[1:] if s not in cr_surnames]
                if mismatched:
                    severity = "CRITICAL" if title_overlap < 0.7 else "WARNING"
                    issues.append({
                        "severity": severity,
                        "message": f"Co-author surname(s) not found in Crossref: {mismatched} "
                                   f"(Crossref authors: {authors_cr[:5]})"
                    })
...
# ─── Report Generation ────────────────────────────────────────────────────────
def generate_markdown_report(results, timestamp, source_name="create_perspective_docx.py"):
    ...
    lines = [
        ...
        f"**Source:** `scripts/{source_name}` REFERENCES list",
...
def main():
    import sys
    source_name = "create_perspective_docx.py"
    report_prefix = "reference_audit_report"
    if len(sys.argv) > 1:
        source_name = sys.argv[1]
        report_prefix = Path(source_name).stem + "_reference_audit_report"
    ...
    # Load references
    refs = load_references(source_name)
    print(f"\nLoaded {len(refs)} references from {source_name}\n")
    ...
    # Markdown report
    md_report = generate_markdown_report(results, timestamp, source_name)
