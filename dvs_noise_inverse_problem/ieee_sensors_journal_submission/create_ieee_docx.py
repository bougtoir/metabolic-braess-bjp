...
def build_manuscript():
    ...
    add_heading(doc, '7.1. Implications', level=2)
    add_paragraph(doc, (
        'The closed form \u03c1* = \u221a(1 \u2212 \u03b8\u00b2/\u03c3\u00b2) does not prescribe removing all noise; '
        'it prescribes reducing the effective noise to the stochastic-resonance peak. '
        'This is a design rule for threshold detectors: stop denoising when the '
        'residual noise is still beneficial, not when it is zero. The A5 circuit '
        'model and the Fano-factor filter translate that rule into a DVS-specific '
        'pipeline. The A5 model treats the pixel current as a Poisson-driven temporal '
        'derivative and converts the global SR condition into a per-pixel covariate '
        'adjustment through the noise-to-threshold ratio. The Fano factor then '
        'separates Poisson-like background activity (F \u2248 1) from bursty signal '
        '(F \u226b 1), and PI-DC-DVS refines the resulting per-event probability with a '
        'small CNN. Because the core classifier uses only event coordinates and '
        'timestamps, the pipeline needs no intensity frames, no IMU, and no '
        'pre-collected clean labels at inference time.'
    ))
    add_paragraph(doc, (
        'On the real EBSSA stream this event-only, calibration-tunable design '
        'recovers faint satellite trajectories. In the proof-of-concept recording it '
        'removes {demo_removal_pct}% of noise while preserving the trajectory (Fig. 7), and the '
        'systematic evaluation gives ROC-AUC = {fano_auc_value}, SPR = {fano_spr_pct}% and '
        'NRR = {fano_nrr_pct}% across {n_valid} of {n_recordings} valid recordings. Those numbers '
        'do not exceed the order-of-magnitude SNR gains reported by recent fully '
        'supervised learning denoisers on curated, labelled benchmarks, but they are '
        'obtained without training data and from the same coordinate-timestamp inputs '
        'that the event-only baselines use. The comparison therefore isolates the '
        'contribution of the SR-optimal stopping rule and the Fano-based background '
        'model, not the contribution of additional sensors or learned priors. From an '
        'engineering standpoint the practical payoff is the {a5_mean_snr}\u00d7 mean SNR gain '
        'predicted by the A5 simulations (up to {a5_max_snr}\u00d7), i.e. about 2.5 magnitudes '
        'in the detection limit for space situational awareness.'
    ))
    add_paragraph(doc, (
        'More importantly, the framework changes how one reasons about DVS noise '
        'removal. Instead of treating every background event as a defect to be erased, '
        'it treats the optimal residual noise as a resource that enables threshold '
        'crossing of faint signals. That viewpoint is consistent with '
        'stochastic-resonance theory and with the hardware observation that noise '
        'events dominate at low light. The six-tier calibration hierarchy, especially '
        'the satellite-transit verification tier, gives an in-operation check that the '
        'noise model remains valid after deployment. Because most computation is '
        'event-driven and the CNN only refines a Fano-based estimate, the approach is '
        'also compatible with edge processing on event cameras. Thus, while the Fano '
        'filter is not the highest-SNR denoiser in absolute terms on labelled '
        'benchmarks, it offers a physically interpretable, no-training and '
        'edge-executable route to near-optimal denoising for threshold-based event '
        'sensors.'
    ))

    add_heading(doc, '7.2. Limitations and future work', level=2)
    add_paragraph(doc, (
