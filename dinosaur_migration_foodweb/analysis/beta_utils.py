...
def pairwise_simpson(mat: np.ndarray) -> np.ndarray:
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            b = mat[j]
            shared = np.minimum(a, b).sum()
            m = min(a.sum(), b.sum())
            out[i, j] = out[j, i] = 1.0 - shared / m if m else 0.0
    return out
...
def pairwise_sorensen(mat: np.ndarray) -> np.ndarray:
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            shared = np.minimum(a, b).sum()
            tot = a.sum() + b.sum()
            out[i, j] = out[j, i] = 1.0 - 2 * shared / tot if tot else 0.0
    return out
...
def mantel(dist: np.ndarray, diss: np.ndarray, n_perm: int, rng) -> tuple[float, float]:
    ...
    for k in range(n_perm):
        p = rng.permutation(n)
        perm[k] = spearmanr(d, diss[np.ix_(p, p)][iu]).statistic
