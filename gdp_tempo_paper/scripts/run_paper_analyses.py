...
def _ols_fit(x: np.ndarray, y: np.ndarray) -> tuple[float, float, float, float]:
    """Return (slope, intercept, r2, t_stat_ols)."""
    if len(x) < 3:
        return np.nan, np.nan, np.nan, np.nan
    x_c = x - x.mean()
    y_c = y - y.mean()
    ...
    if denom <= 0:
        return np.nan, np.nan, np.nan, np.nan
    slope = float(np.sum(x_c * y_c) / denom)
    ...
    se_beta = se / np.sqrt(denom) if denom > 0 else np.nan
    t_stat = slope / se_beta if np.isfinite(se_beta) and se_beta > 0 else np.nan
    return slope, intercept, r2, t_stat


def _robust_se(x: np.ndarray, y: np.ndarray, slope: float, intercept: float) -> float:
    """Huber-White HC1 standard error for the slope."""
    n = len(x)
    if n <= 2:
        return np.nan
    x_c = x - x.mean()
    ...
    if denom <= 0:
        return np.nan
    var = (n / (n - 2)) * float(np.sum(x_c ** 2 * resid ** 2)) / (denom ** 2)
...
def run_rho2_regression(rpim: pd.DataFrame,
    ...
    Reports OLS and heteroskedasticity-robust standard errors, Spearman
    rank correlation, and leave-one-out sensitivity to strengthen the
    cross-sectional evidence on asset-composition determinants of RPIM
    diagnostics.
    """
    ...
    for model in ("M0", "M4"):
        y = merged[f"{model}_rho2"].values.astype(float)
        x = merged["rnd_intensity"].values.astype(float)
        n = len(y)
        if n < 5:
            continue

        slope, intercept, r2, t_ols = _ols_fit(x, y)
        robust_se = _robust_se(x, y, slope, intercept)
        t_robust = slope / robust_se if robust_se > 0 else np.nan
        p_robust = float(2 * stats.t.sf(abs(t_robust), n - 2)) \
            if np.isfinite(t_robust) else np.nan
        ...
        for i in range(n):
            x_loo = np.delete(x, i)
            y_loo = np.delete(y, i)
            slope_loo, _, _, _ = _ols_fit(x_loo, y_loo)
            if np.isfinite(slope_loo):
                loo_slopes.append(float(slope_loo))
                # Use robust p-value on the LOO sample
                se_loo = _robust_se(x_loo, y_loo, slope_loo,
                                    y_loo.mean() - slope_loo * x_loo.mean())
                t_loo = slope_loo / se_loo if se_loo > 0 else np.nan
                p_loo = float(2 * stats.t.sf(abs(t_loo), len(x_loo) - 2)) \
                    if np.isfinite(t_loo) else np.nan
                loo_pvals.append(p_loo)
                loo_countries.append(str(merged.iloc[i]["iso3"]))

        loo_arr = np.array(loo_slopes)
        loo_parr = np.array(loo_pvals)
        n_sig = int(np.sum(loo_parr < 0.05)) if len(loo_parr) else 0
        influential = [loo_countries[i] for i, p in enumerate(loo_parr)
                       if p >= 0.05] if len(loo_parr) else []

        result[f"{model}_intercept"] = float(intercept)
        result[f"{model}_slope"] = float(slope)
        result[f"{model}_R2"] = float(r2)
        result[f"{model}_t_stat"] = float(t_robust)
        result[f"{model}_t_stat_ols"] = float(t_ols)
        result[f"{model}_robust_se"] = float(robust_se)
        result[f"{model}_p_value"] = float(p_robust)
        if hasattr(spearman, "correlation"):
            rho_s, p_s = float(spearman.correlation), float(spearman.pvalue)
        elif hasattr(spearman, "statistic"):
            rho_s, p_s = float(spearman.statistic), float(spearman.pvalue)
        else:
            rho_s, p_s = float(spearman[0]), float(spearman[1])
        result[f"{model}_spearman_rho"] = rho_s
        result[f"{model}_spearman_p"] = p_s
        result[f"{model}_n"] = n
        result[f"{model}_loo_slope_mean"] = float(np.mean(loo_arr)) if len(loo_arr) else np.nan
        result[f"{model}_loo_slope_std"] = float(np.std(loo_arr, ddof=1)) if len(loo_arr) > 1 else np.nan
        result[f"{model}_loo_slope_min"] = float(np.min(loo_arr)) if len(loo_arr) else np.nan
        result[f"{model}_loo_slope_max"] = float(np.max(loo_arr)) if len(loo_arr) else np.nan
        result[f"{model}_loo_n_sig"] = n_sig
        result[f"{model}_loo_n"] = len(loo_arr)
        result[f"{model}_loo_influential"] = influential
        print(f"  [rho2_reg] {model}: slope={slope:.4f} "
              f"R2={r2:.3f} t_robust={t_robust:.2f} "
              f"spearman={rho_s:.3f} "
              f"LOO_sig={n_sig}/{len(loo_arr)}", flush=True)
...
def make_figures(fair: pd.DataFrame, oos: pd.DataFrame,
    ...
    if not fair.empty:
        ...
        ax.set_title("In-sample growth-rate fit across five K-constructions")
        ax.legend(loc="lower right")
        plt.tight_layout()
        plt.savefig(os.path.join(FIG, "fig1_m_ranking.png"), dpi=300)
        plt.close()
    ...
    if not oos.empty:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(FIG, "fig2_oos.png"), dpi=300)
        plt.close()
    ...
    if not boot.empty:
        ...
        fig.suptitle("Bootstrap 95% CI for joint-identified (μ, β)")
        plt.tight_layout()
        plt.savefig(os.path.join(FIG, "fig3_bootstrap.png"), dpi=300)
        plt.close()
    ...
    if not gamma.empty:
        ...
        ax.set_title("γ_price sensitivity of the PIM-CWON gap")
        ax.legend()
        ax.grid(alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(FIG, "fig4_gamma_price.png"), dpi=300)
        plt.close()
...
def main():
    ...
    # M2 vs M0 out-of-sample MAPE improvement significance
    if "M0_oos_mape" in oos.columns and "M2_oos_mape" in oos.columns:
        paired = oos[["M0_oos_mape", "M2_oos_mape"]].dropna()
        if len(paired) >= 6:
            try:
                stat, p = stats.wilcoxon(
                    paired["M0_oos_mape"], paired["M2_oos_mape"],
                    alternative="two-sided", zero_method="zsplit")
                oos_summary["m2_vs_m0_wilcoxon_stat"] = float(stat)
                oos_summary["m2_vs_m0_wilcoxon_p"] = float(p)
                oos_summary["m2_vs_m0_n_pairs"] = int(len(paired))
                oos_summary["m2_vs_m0_median_diff"] = float(
                    (paired["M0_oos_mape"] - paired["M2_oos_mape"]).median())
                print(f"  M2 vs M0 Wilcoxon: stat={stat:.2f}, "
                      f"p={p:.4g}, n={len(paired)}", flush=True)
            except Exception:
                pass
