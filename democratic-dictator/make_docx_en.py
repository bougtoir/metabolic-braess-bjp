...
for line in lines[body_start:]:
    ...
    if re.match(r'^\s{4,}', line) and not stripped.startswith('where'):
        ...
        continue
    # Figure markers: inline figures for working manuscript
    fig_match = FIG_RE.match(s)
    if fig_match:
        path = fig_match.group(1).strip()
        img_path = ROOT / path
        try:
            p = doc.add_paragraph()
            p.alignment = WD_ALIGN_PARAGRAPH.CENTER
            ...
            add_para(doc, f'[figure not found: {img_path} ({e})]')
        continue
    # Numbered headings
    ...
    if SUBSECTION_RE.match(s):
        ...
        continue
    # Bold title-page/front-matter headings
    if TITLE_LINE_RE.match(s):
        add_para(doc, s, bold=True)
        continue
    # Figure captions / centered lines
