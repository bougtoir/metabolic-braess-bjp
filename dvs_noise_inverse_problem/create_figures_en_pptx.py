...
def create_gap_map_figure_en():
    ...
    save_figure(fig, out, facecolor='white')
    plt.close()
    return out
...
def create_pptx(fig_files):
    ...
    for fig_path, title, caption in zip(fig_files, titles, captions):
        ...
        max_w = Inches(11)
        max_h = Inches(5.5)
        scale = min(max_w / Emu(int(img_w * 914400 / DPI)),
                    max_h / Emu(int(img_h * 914400 / DPI)))
        final_w = int(img_w * 914400 / DPI * scale)
        final_h = int(img_h * 914400 / DPI * scale)
